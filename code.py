...
def get_data(**kwargs):
    ...
    train_data_input[:, :, 10:18, 10:18] = 0                    #loads data converts it into tensors and mask the center of the images
...
def train_model(train_data_input, train_data_label, **kwargs):          #creates instance of our model class to predict the masked area of the images
    """
    ...
    criterion = nn.MSELoss()                                                            #mean squared error loss to measure the loss
    # TODO: Dummy optimizer - change this to a more suitable optimizer
    optimizer = torch.optim.Adam(model.parameters(), weight_decay=5e-5)                 #Adam optimiser updates the model's weights based on the gradients of the loss function
    ...
    batch_size = 128                                                                #load data into batches to train more effectively
    ...
    dataset = TensorDataset(train_data_input, train_data_label)
    val_rat = 0.1
    n_value = int(len(dataset) * val_rat)
    n_tr = len(dataset) - n_value
    train_set, val_set = random_split(dataset, [n_tr, n_value])
    ...
    for epoch in range(n_epochs):                                                   #train the model and measure its performance
        model.train()
        running_loss = 0.0
        ...
        averge_tr_loss = running_loss / len(train_loader)
        tr_losses.append(averge_tr_loss)
        ...
        with torch.no_grad():
            for x_val, y_val in val_loader:
                ...
                val_loss += loss.item()
        averge_value_loss = val_loss / len(val_loader)
        value_losses.append(averge_value_loss)

        print(f"Epoch {epoch+1:03d} | Train Loss: {averge_tr_loss:.6f} | Val Loss: {averge_value_loss:.6f}")
...
class Model(nn.Module):
    ...
    def __init__(self):
        ...
        # Encoder: Feature extraction and compression
        self.encoder = nn.Sequential(                                                   #define the encoder as a sequence of layers
            nn.Conv2d(1, 8, kernel_size=3, stride=1, padding=1),   # [B, 8, 28, 28]     #image input, applied 8 3x3 filters
            nn.LeakyReLU(),                                                             #activation function, prevents "dead neurons"
            nn.MaxPool2d(2, 2),                                    # [B, 8, 14, 14]

            nn.Conv2d(8, 16, kernel_size=3, stride=1, padding=1),  # [B, 16, 14, 14] #8 input channels and 16 new filters
            nn.LeakyReLU(),
            nn.Flatten(),                                          # [B, 16*14*14]

            nn.Linear(16 * 14 * 14, 256),                               #compress the size of the input
            nn.LeakyReLU(),
            nn.Dropout(p=0.3),                                          #set some inputs to zero to prevent overfitting

            nn.Linear(256, 64),                                    # [B, 64] compress
            nn.LeakyReLU(),
            nn.Unflatten(1, (64, 1, 1)),                           # [B, 64, 1, 1]
        )

        # Decoder: Upsample and reconstruct image  from the 64 compressed values from the encoder
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(64, 32, kernel_size=7, stride=1),   # [B, 32, 7, 7]
            nn.LeakyReLU(),
            ...
            nn.Sigmoid(),  # Normalize output to [0, 1] pixel values in this range
        )
...
def test_model(model, test_data_input):                     #uses the model to make predictions
                                                            #saves the results in the required format
    """
